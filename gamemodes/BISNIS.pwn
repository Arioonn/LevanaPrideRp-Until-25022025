...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
		...
		case 6:
        {
            format(string, sizeof(string), "Produk\tHarga\n{ffffff}Wine\t{7fff00}%s\n{ffffff}Beer\t{7fff00}%s\n{ffffff}Cocktail\t{7fff00}%s\n{ffffff}Ice lemon\t{7fff00}%s",
			    FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3]),
...
Bisnis_ProductMenu(playerid, bizid)
{
    ...
    switch (bData[bizid][bType])
    {
		...
		case 7:
        {
			format(string, sizeof(string), "Produk\tHarga\n{ffffff}Boxing\t{7fff00}%s\n{ffffff}Grabkick\t{7fff00}%s\n{ffffff}Kneehead\t{7fff00}%s\n{ffffff}Kungfu\t{7fff00}%s\t{7fff00}%s\t{7fff00}%s",
			    FormatMoney(bData[bizid][bP][0]),
                FormatMoney(bData[bizid][bP][1]),
                FormatMoney(bData[bizid][bP][2]),
                FormatMoney(bData[bizid][bP][3]),
...
Bisnis_Type(bisid)
{
	...
	if(bData[bisid][bType] == 5) // Electronic
	{
		switch(random(2))
		{
			case 0:
			{
				bData[bisid][bIntposX] = -2240.78;
				bData[bisid][bIntposY] = 137.209;
				bData[bisid][bIntposZ] = 1035.71;
				bData[bisid][bIntposA] = 89.6855;
				bData[bisid][bInt] = 6;
			}
			case 1:
			{
	...
	if(bData[bisid][bType] == 6) // Bar
	{
		switch(random(2))
		{
			case 0:
			{
				bData[bisid][bIntposX] = 501.941;
				bData[bisid][bIntposY] = -67.5638;
				bData[bisid][bIntposZ] = 999.058;
				bData[bisid][bIntposA] = 180;
				bData[bisid][bInt] = 11;
			}
			case 1:
			{
	...
	if(bData[bisid][bType] == 7) // GYM
	{
		switch(random(2))
		{
			case 0:
			{
				bData[bisid][bIntposX] = 772.288;
				bData[bisid][bIntposY] = -5.51454;
				bData[bisid][bIntposZ] = 1001.03;
				bData[bisid][bIntposA] = 176.465;
				bData[bisid][bInt] = 5;
			}
			case 1:
			{
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		format(tstr, 128, ""LG_E"use '/buy' here");
		bData[id][bLabelTrucker] = CreateDynamic3DTextLabel(tstr, COLOR_YELLOW, bData[id][bPointX], bData[id][bPointY], bData[id][bPointZ]+0.5, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, id, bData[id][bInt]);
...
CMD:createbiz(playerid, params[])
{
	...
	new price, type;
	if(sscanf(params, "dd", price, type)) return Usage(playerid, "/createbusiness [price] [type, 1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronics 6.Bar 7.GYM]");
...
CMD:editbiz(playerid, params[])
{
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", btype))
            return Usage(playerid, "/editbisnis [id] [Type] [1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronics 6.Bar]");
...
CMD:mybs(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FFFFFF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
