...
CMD:fedit(playerid, params[])
{
    ...
    else if(!strcmp(type, "marijuana", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family marijuana ID: %d to %s.", pData[playerid][pAdminname], fid, marijuana);
    }
	else if(!strcmp(type, "component", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family component ID: %d to %s.", pData[playerid][pAdminname], fid, comp);
    }
	else if(!strcmp(type, "material", true))
    {
		...
		SendStaffMessage(COLOR_LIGHTRED, "Admin %s has changed the family material ID: %d to %s.", pData[playerid][pAdminname], fid, mat);
    }
    return 1;
}
...
CMD:finvite(playerid, params[])
{
    ...
    if(sscanf(params, "u", otherid))
        return Error(playerid, "/invite [playerid/PartOfName]");
...
CMD:funinvite(playerid, params[])
{
	...
	new otherid;
    if(sscanf(params, "u", otherid))
        return Error(playerid, "/uninvite [playerid/PartOfName]");
...
CMD:fsetrank(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, rank))
        return Error(playerid, "/fsetrank [playerid/PartOfName] [rank 1-6]");
...
CMD:f(playerid, params[])
{
    ...
    if(sscanf(params,"s[128]",text))
        return Error(playerid, "/f(family) [text]");
