...
CMD:asalary(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
		return Usage(playerid, "/asalary [playerid]");

	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Playerid tidak di temukan!");

	AdisplaySalary(playerid, otherid);
	Servers(playerid, "Anda berhasil mengecek Salary %s", ReturnName(otherid));
	return 1;
}
...
{
	new query[512];
	mysql_format(g_SQL, query, sizeof(query), "SELECT * FROM salary WHERE owner='%d' ORDER BY id ASC LIMIT 30", pData[otherid][pID]);
	...
	if(rows) 
	{
		...
		totalduty = pData[otherid][pOnDutyTime] + pData[otherid][pTaxiTime];
		if(totalduty > 600)
		{
			gajiduty = 600;
		}
		...
		if(pData[otherid][pJob] == 1 || pData[otherid][pJob2] == 1)
		{
		...
		new title[48];
		format(title, sizeof(title), "Salary: %s", pData[otherid][pName]);
