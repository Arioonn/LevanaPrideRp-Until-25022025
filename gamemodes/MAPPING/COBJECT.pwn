...
CMD:setmaterialobject(playerid, params[])
{
...
CMD:cloneobject(playerid, params[]) {
    new objectid, Float:ox, Float:oy, Float:oz;
    if(pData[playerid][pAdmin] < 5) return PermissionError(playerid);
    
    // Parse the command parameter to get the object ID
    if(sscanf(params, "i", objectid)) return Usage(playerid, "/cloneobject [objectid]");
    
    // Check if the object ID is valid
    if(IsValidDynamicObject(objectid)) {
        // Get the position and rotation of the existing object
        GetDynamicObjectPos(objectid, ox, oy, oz);
        GetDynamicObjectRot(objectid, ox, oy, oz);
        
        // Find a free index for the new object
        for(new idx = 1; idx < MAX_COBJECTS; idx++) {
            if(!ObjectInfo[idx][oModel]) {
                // Set the object information to the new index
                ObjectInfo[idx][oModel] = ObjectInfo[objectid][oModel];
                ObjectInfo[idx][oX] = ox + 2.0; // Offset to prevent overlapping
                ObjectInfo[idx][oY] = oy + 2.0; // Offset to prevent overlapping
                ObjectInfo[idx][oZ] = oz;
                ObjectInfo[idx][oRX] = ObjectInfo[objectid][oRX];
                ObjectInfo[idx][oRY] = ObjectInfo[objectid][oRY];
                ObjectInfo[idx][oRZ] = ObjectInfo[objectid][oRZ];
                ObjectInfo[idx][oMT] = ObjectInfo[objectid][oMT];
                
                // Create the new dynamic object
                ObjectInfo[idx][obj] = CreateDynamicObject(ObjectInfo[idx][oModel], ObjectInfo[idx][oX], ObjectInfo[idx][oY], ObjectInfo[idx][oZ], ObjectInfo[idx][oRX], ObjectInfo[idx][oRY], ObjectInfo[idx][oRZ]);
                
                // Optional: create a label to the object (if needed)
                // ObjectInfo[idx][oText] = CreateDynamic3DTextLabel("Your Text Here", COLOR_RED, ObjectInfo[idx][oX], ObjectInfo[idx][oY], ObjectInfo[idx][oZ], 10.0, INVALID_PLAYER_ID, INVALID_PLAYER_ID, DYNAMIC_RANGE_CHECK_NONE, 200.0, worldid, interiorid);
				                // Log the object creation for staff
                new String[128];
                format(String, sizeof(String), "AdmCmd: %s cloned object ID %d. (Object Model: %d)", GetName(playerid), idx, ObjectInfo[idx][oModel]);
                SendStaffMessage(COLOR_RED, String, 2);

                // Save object data if needed
                SaveObj();

                break; // Exit the loop once the object is cloned
            }
        }
    } else {
        SendClientMessage(playerid, COLOR_WHITE, "The object ID is not valid or not created by CreateDynamicObject.");
    }
    return 1;
}
