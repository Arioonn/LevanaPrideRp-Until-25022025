...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	{
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX,"Welcome to Levana Pride","{ffffff}This account is {ff0000}Unregistered\n\n{ffffff}Please create a UCP account on Discord first {FFFF00}Levana Pride Roleplay","Close","");
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "gymvip", pData[playerid][pGymVip]);
	cache_get_value_name_int(0, "gym_time", pData[playerid][pGymTime]);
	...
	cache_get_value_name_int(0, "skateboard", pData[playerid][pSkate]);
	...
	cache_get_value_name_int(0, "sudahrent", pData[playerid][pSudahrent]);
	...
	cache_get_value_name_int(0, "sidejob_kurirtime", pData[playerid][pDelayCourier]);
	...
	cache_get_value_name_int(0, "nasgorGocar", pData[playerid][pNasgor]);
	cache_get_value_name_int(0, "susuGocar", pData[playerid][pSusuGO]);
	cache_get_value_name_int(0, "stress", pData[playerid][pBladder]);
	...
	cache_get_value_name_int(0, "timeanddate", pData[playerid][pTimeAndDate]);
	cache_get_value_name_int(0, "servername", pData[playerid][pServerName]);
	cache_get_value_name_int(0, "moneydot", pData[playerid][pMoneyDot]);
	...
	cache_get_value_name_int(0, "createsell", pData[playerid][pCreateSell]);
	cache_get_value_name_int(0, "tagstime", pData[playerid][pTagsTime]);
	//new query[200];
	...
	ClearChat(playerid);
	SendClientMessageEx(playerid, COLOR_ARWIN, "SERVER: "WHITE_E"Selamat datang "YELLOW_E"%s "WHITE_E"kota Levana Pride Roleplay", pData[playerid][pName]);
	SendClientMessageEx(playerid, COLOR_ARWIN, "SERVER: "YELLOW_E"Jika Anda mengalami bug silahkan gunakan command "RED_E"'/stuck'"YELLOW_E"atau "RED_E"'/report'", pData[playerid][pLastLogin]);
	SendClientMessageEx(playerid, COLOR_ARWIN, "SERVER: "YELLOW_E"Gunakan command "RED_E"'/maxobject' "YELLOW_E"untuk mengatur jarak object!");
	...
	foreach(new ii : Player)
	{
		...
		{
			SendClientMessageEx(ii, COLOR_RED, "%s[%d] "YELLOW_E" connected to the server.", pData[playerid][pName], playerid);
		}
	}
	return 1;
}
...
function CekNamaDobelJing(playerid, name[])
{
	...
	if(rows > 0)
	{
		ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, "Create Character", "{BABABA}Read before making a name\n\n{BABABA}[+]Don't use number\n{BABABA}[+]Don't use two cultures\n{BABABA}[+]Don't use words\n\nRead the rules above before creating a character name.\n{ffff00}Example: {BABABA}Vendy_Martinez, Miguel_Gerardo\n\n{b22222}The name has been used by another player. Please use another name.", "Create", "Back");
	}
	else 
	{
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1685.59, -2239.47, 13.8469);
		SetPlayerFacingAngle(playerid, 180.95);
...
SetPlayerStreamerSettings(playerid) {
    switch (pData[playerid][pStreamer]) {
        ...
        case 1: {
            Streamer_SetVisibleItems(STREAMER_TYPE_OBJECT, 200, playerid);
            Streamer_SetRadiusMultiplier(STREAMER_TYPE_OBJECT, 0.2, playerid);
        }
        case 2: {
            Streamer_SetVisibleItems(STREAMER_TYPE_OBJECT, 399, playerid);
