...
CMD:editdealer(playerid, params[])
{
	...
	else if(!strcmp(type, "owner", true))
    {
		...
		DealerData[did][dVisit] = gettime() + (86400 * 30);
		DealerSave(did);
		DealerRefresh(did);
...
// 			if(strcmp(DealerData[bid][dealerOwnerID], "-"))
// 				return Error(playerid, "Someone already owns this dealership.");
...
// 			DealerData[bid][dVisit] = gettime() + (86400 * 30);
// 			DealerRefresh(bid);
// 			DealerSave(bid);
// 		}
// 	}
        ...
        if (DealerData[id][dealerPosX] != 0 && DealerData[id][dealerPosY] != 0 && DealerData[id][dealerPosZ] != 0)
        {
            if (strcmp(DealerData[id][dealerOwner], "-") != 0 || DealerData[id][dealerOwnerID] != 0)
            {
                format(tstr, sizeof(tstr), "[ID: %d]\n" WHITE_E "Name: {FFFF00}%s\n" WHITE_E "Owned by %s\nTax Time: " YELLOW_E "%s\nType: " YELLOW_E "%s\n" WHITE_E "Use " RED_E "/buypv " WHITE_E "to buy vehicle in this dealership",
                       id, DealerData[id][dealerName], DealerData[id][dealerOwner], ReturnTimelapse(gettime(), DealerData[id][dVisit]), type);
            }
            else
            {
...
DealerBuyVehicle(playerid, dealerid)
{
    ...
    switch(DealerData[dealerid][dealerType])
    {
        ...
        case 2:
        {
			...
			GetVehicleModelName(436), FormatMoney(DealerData[dealerid][dP][4]),
			GetVehicleModelName(466), FormatMoney(DealerData[dealerid][dP][5]),
        ...
        case 4:
		{
			...
			GetVehicleModelName(413), FormatMoney(DealerData[dealerid][dP][3]),
			GetVehicleModelName(414), FormatMoney(DealerData[dealerid][dP][4]),
			GetVehicleModelName(422), FormatMoney(DealerData[dealerid][dP][5]),
			GetVehicleModelName(440), FormatMoney(DealerData[dealerid][dP][6]),
			GetVehicleModelName(455), FormatMoney(DealerData[dealerid][dP][7]),
			GetVehicleModelName(456), FormatMoney(DealerData[dealerid][dP][8]),
			GetVehicleModelName(478), FormatMoney(DealerData[dealerid][dP][9])
			);
...
function OnVehDealer(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a)
{
	...
	Servers(playerid, "Anda telah membeli kendaraan dengan model %s(%d)", GetVehicleModelName(model), model);
