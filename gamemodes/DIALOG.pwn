...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			{
				new lstring[300];
				format(lstring, sizeof lstring, "{ffffff}This account is {32cd32}Registered\n{ffffff}User control panel: {15D4ED}%s\n\n{ffffff}You still have {FFFF00}%i/3 altempt(s)\n{ffffff}You have 60 second, so please input your password\n\n{b22222}The password you entered is incorrect, please enter it again", pData[playerid][pUCP], pData[playerid][LoginAttempts]);
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Welcome to Levana Pride", lstring, "Login", "Abort");
			}
		}
        return 1;
    }
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Levana Pride", "Kata sandi minimal 5 Karakter!\nMohon isi Password dibawah ini:", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Levana Pride", "Kata sandi yang anda gunakan mengandung karakter yang valid!\nMohon isi Password dibawah ini:", "Register", "Tolak");
			return 1;
		}
	...
	if(dialogid == DIALOG_CHARLIST)
    {
		if(response)
		{
			if(PlayerChar[playerid][listitem][0] == EOS)
				return ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, "Create Character",  "{BABABA}Read before making a name\n\n{BABABA}[+]Don't use number\n{BABABA}[+]Don't use two cultures\n{BABABA}[+]Don't use words\n\nRead the rules above before creating a character name.\n{ffff00}Example: {BABABA}Vendy_Martinez, Miguel_Gerardo", "Create", "Back");
	...
	if(dialogid == DIALOG_MAKE_CHAR)
	{
	    if(response)
	    {
			...
			InterpolateCameraPos(playerid, -1.9223, -1240.7756, 117.9103, 1142.4589, -717.8170, 139.2966, 25000);
			InterpolateCameraLookAt(playerid, 186.6470, -1261.1404, 78.2794, 1280.0114, -640.1159, 106.2128, 25000);
	...
	if(dialogid == DIALOG_GENDER)
    {
		...
		if(response)
		{
			...
			switch (listitem) 
			{
				case 0: 
				{
					SendClientMessageEx(playerid,COLOR_ARWIN,"SERVER: {FFFFFF}Registrasi Berhasil! Terima kasih telah bergabung ke dalam server!");
					SendClientMessageEx(playerid,COLOR_ARWIN,"SERVER: {FFFFFF}Tanggal Lahir : %s | Gender : Male/Laki-Laki", pData[playerid][pAge]);
					ShowPlayerDialog(playerid, DIALOG_SPAWN_1, DIALOG_STYLE_LIST, "Location", "Los Santos Airport", "Select", "Cancel");
					
				}
				case 1: 
				{
					SendClientMessageEx(playerid,COLOR_ARWIN,"SERVER: {FFFFFF}Registrasi Berhasil! Terima kasih telah bergabung ke dalam server!");
					SendClientMessageEx(playerid,COLOR_ARWIN,"SERVER: {FFFFFF}Tanggal Lahir : %s | Gender : Female/Perempuan", pData[playerid][pAge]);
					ShowPlayerDialog(playerid, DIALOG_SPAWN_1, DIALOG_STYLE_LIST, "Location", "Los Santos Airport", "Select", "Cancel");
	...
	if(dialogid == DIALOG_HUDMODE)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialog(playerid, DIALOG_HBEMODE, DIALOG_STYLE_LIST, "HBE Mode", "Modern\nDisable\nSimple\nDisable", "Confirm", "Close");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, DIALOG_TIMEANDDATE, DIALOG_STYLE_LIST, "HUD Time/Date", "Show\nHide", "Confirm", "Close");
				}
				case 2:
				{
				...
				case 3:
				{
					ShowPlayerDialog(playerid, DIALOG_MONEYDOT, DIALOG_STYLE_LIST, "HUD Money Dot", "Show\nHide", "Confirm", "Close");
				}
			}
		}
	}
	if(dialogid == DIALOG_SETTINGS)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 2:
				{	
					return callcmd::hudmode(playerid);
				}
				case 3:
				{
					return callcmd::togglesettings(playerid);
				}
				case 4:
				{
					return callcmd::maxobject(playerid);
				}
			}
		}
	}
	if(dialogid == DIALOG_TIMEANDDATE)
	{
		if(response)
		{
			switch(listitem)
			...
			switch (listitem)
			{
				case 0:
				{
					...
					else
					{
						...
						SendFactionMessage(5, COLOR_RADIO, "PEDAGANG: [%s] sekarang kembali on duty...", ReturnName(playerid));
					}
				}
				case 1:
				{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_GUDANGPEDAGANG, DIALOG_STYLE_LIST, "PEDAGANG", "Burger\nSprunk", "Pilih", "Batal");
				}
				case 4:
				{
					if(pData[playerid][pOnDuty] <= 0)
					...
					{
						//case 1: ShowModelSelectionMenu(playerid, PEDMale, "Choose your skin");
						//case 2: ShowModelSelectionMenu(playerid, PEDFemale, "Choose your skin");
						case 1: ShowPlayerSelectionMenu(playerid, GOCAR_SKIN_MALE, "Choose Your Skin", GOCARSkinMale, sizeof(GOCARSkinMale));
						case 2: ShowPlayerSelectionMenu(playerid, GOCAR_SKIN_FEMALE, "Choose Your Skin", GOCARSkinFemale, sizeof(GOCARSkinFemale));					
					}
				}
			}
		}
	...
	if(dialogid == DIALOG_TAKEFOOD)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					pData[playerid][pSnack] += 5;//pData[playerid][pWheats] += 10;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli snack dari dapur go food", ReturnName(playerid));
				}
				case 1:
				{
					pData[playerid][pSprunk] += 5;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli Sprunk dari dapur go food", ReturnName(playerid));
				}
				case 2:
				{
					pData[playerid][pNasgor] += 5;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli Nasi goreng dari dapur go food", ReturnName(playerid));
				}
				case 3:
				{
					pData[playerid][pSusuGO] += 5;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli Susu dari dapur go food", ReturnName(playerid));
				}
			}
		}
		return 1;
		...
		if(response)
		{
			switch (listitem)
			{
				case 0:
				{
					...
					Pedagang -= 10;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s telah mengambil Snack 10 dari Gudang", ReturnName(playerid));
					// new dc[128];
					// format(dc, sizeof(dc),  "```%s Telah Mengambil Susu Di Locker SAPM```", ReturnName(playerid));
					// SendDiscordMessage(14, dc);
				}
				case 1:
				{
					if(Pedagang < 1) return Error(playerid, "Product out of stock!");
					...
					Pedagang -= 10;
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s telah mengambil Sprunk 10 dari Gudang", ReturnName(playerid));
					// new dc[128];
					// format(dc, sizeof(dc),  "```%s Telah Mengambil Kebab Di Locker SAPM```", ReturnName(playerid));
					// SendDiscordMessage(14, dc);
				}
			}
		}
	...
	if(dialogid == DIALOG_EMOTEPROPERTY)
	{
		if(response)
		{
			switch(listitem)
			{
                ...
                case 2:
				{
                    SetPlayerAttachedObject(playerid, 9, 1518, 1,  0.212000, 0.453999, -0.052000,  -170.999923, 81.999969, -11.500000,  0.687999, 0.830000, 0.731999);
		            ApplyAnimation(playerid, "CARRY", "crry_prtial", 4.0, 1, 1, 1, 1, 1);
				}
                //basket
                case 3:
				{
                    SetPlayerAttachedObject(playerid, 9, 3065, 6,  0.199000, 0.002999, 0.025000,  0.000000, 0.000000, 0.000000,  1.177000, 1.055000, 1.048000);
		            ApplyAnimation(playerid, "BSKTBALL", "BBALL_idleloop", 4.0, 1, 1, 1, 1, 1);
				}
                //kursi
                case 4:
				{
                    SetPlayerAttachedObject(playerid, 9, 2121, 1,  -0.219999, 0.001999, -0.025000,  -176.699996, 88.500000, -5.499999,  1.000000, 1.000000, 1.000000);
	                ApplyAnimation(playerid, "ped", "SEAT_down", 4.0, 0, 0, 0, 1, 1, 1);
				}
                //payung
                case 5:
				{
                    SetPlayerAttachedObject(playerid, 9, 642, 1,  0.518999, 0.130000, -0.122999,  -89.399986, 99.799926, 6.699999,  0.202000, 0.221000, 0.271999);
		            ApplyAnimation(playerid,"CAMERA","camstnd_idleloop", 4.1, 1, 1, 1, 1, 1, 1);
				}
                //bendera
                case 6:
				{
                    SetPlayerAttachedObject(playerid, 9, 11245, 5,  0.250000, 0.055999, -0.217999,  -2.000001, -123.199966, 177.900009,  0.239000, 0.236000, 0.293000);
	                ApplyAnimation(playerid, "PED", "Jetpack_Idle", 4.1, 0, 1, 1, 1, 0, 1);
				}
                //bunga
                case 7:
				{
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 1: //HBE Hide
				{
					...
					PlayerTextDrawHide(playerid, BLADDER[playerid]);
					PlayerTextDrawHide(playerid, hbeNew[playerid][0]);
					PlayerTextDrawHide(playerid, hbeNew[playerid][1]);
					PlayerTextDrawHide(playerid, hbeNew[playerid][2]);
					PlayerTextDrawHide(playerid, hbeNew[playerid][3]);
				}
				case 2: //HBE Simple
				{
				...
				case 3: //HBE Simple
				{
					...
					PlayerTextDrawHide(playerid, HbeJgrpCuy[playerid][0]);
                    PlayerTextDrawHide(playerid, HbeJgrpCuy[playerid][2]);
                    PlayerTextDrawHide(playerid, HbeJgrpCuy[playerid][3]);
			...
			else if(bData[bizid][bType] == 2)
			{
				switch(listitem)
				{
					...
					case 4:
					{
						...
						GivePlayerMoneyEx(playerid, -price);
						pData[playerid][pSkate] += 1;
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli Skateboard seharga %s.", ReturnName(playerid), FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"Levana Pride RP: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						if(price == 0)
			...
			if(bData[bizid][bType] == 6)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						//SetPlayerSpecialAction(playerid,SPECIAL_ACTION_DRINK_SPRUNK);
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli minuman %s dan langsung meminumnya", ReturnName(playerid), FormatMoney(price));
						//SetPVarInt(playerid, "UsingSprunk", 1);
					}
					case 1:
					{
		...
		{
			{
				switch(listitem)
				{
					case 0:
					{
						...
						pData[playerid][pGymTime] = gettime() + 259200; // 3 hari dalam detik
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s kamu telah membeli GYM Membership 2 Days dengan membayar uang %s", ReturnName(playerid), FormatMoney(price));
					}
					case 1:
					{
						if(price == 0)
						...
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s kamu telah berlatih Boxing dengan membayar uang %s", ReturnName(playerid), FormatMoney(price));
					}
					case 2:
					{
						if(price == 0)
						...
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s kamu telah berlatih Grabkick dengan membayar uang %s", ReturnName(playerid), FormatMoney(price));
					}
					case 3:
					{
						if(price == 0)
						...
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s kamu telah berlatih Kneehead dengan membayar uang %s", ReturnName(playerid), FormatMoney(price));
					}
					case 4:
					{
						if(price == 0)
						...
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s kamu telah berlatih Kungfu dengan membayar uang %s", ReturnName(playerid), FormatMoney(price));
					}
				}
			}
		}
	...
	{
		if(Player_OwnsDealer(playerid, pData[playerid][pInDealer]))
		{
			if(response)
			{
				static
				...
				return callcmd::dealermanage(playerid, "\0");
		}
		return 1;
	}
	if(dialogid == DEALER_PRICESET)
	{
		...
		if(Player_OwnsDealer(playerid, pData[playerid][pInDealer]))
		{
			if(response)
			{
				...
				if(strval(inputtext) < 1 || strval(inputtext) > 15000)
				{
					new str[128];
					format(str,sizeof(str), "Please enter the new product price for %s ($1 to $7000):", item);
					ShowPlayerDialog(playerid, DEALER_PRICESET, DIALOG_STYLE_INPUT, "Dealer: Set Price", str, "Modify", "Back");
					return 1;
				}
				DealerData[dealerid][dP][pData[playerid][pProductModify]] = strval(inputtext);
				DealerSave(dealerid);
	...
	if(dialogid == BISNIS_PRICESET)
	{
		...
		if(Player_OwnsBisnis(playerid, pData[playerid][pInBiz]))
		{
			if(response)
			{
				...
				{
					new str[128];
					format(str,sizeof(str), "Please enter the new product price for %s ($1 to $500):", item);
					ShowPlayerDialog(playerid, BISNIS_PRICESET, DIALOG_STYLE_INPUT, "Business: Set Price", str, "Modify", "Back");
					return 1;
				}
	...
	if(dialogid == HOUSE_STORAGE)
	{
		...
		if(response)
		{
			...
			{
				format(string, sizeof(string), "Type\tSkin\nClothes 1:\t%d\nClothes 2:\t%d\nClothes 3:\t%d", hData[hid][hSkin1], hData[hid][hSkin2], hData[hid][hSkin3]);
				ShowPlayerDialog(playerid, HOUSE_CLOTHES, DIALOG_STYLE_TABLIST_HEADERS, "House Clothes", string, "Select", "Back");
			} 
		}
		return 1;
	}
	...
	if(dialogid == ST_WEAPON)
	{
		new stid = pData[playerid][pInSt];
		if(!IsStorageOwner(playerid, pData[playerid][pInSt])) 
		    return Error(playerid, "You dont own this Storage.");
				
		if(response)
		{
			if(stData[stid][stWeapon][listitem] != 0)
			{
				GivePlayerWeaponEx(playerid, stData[stid][stWeapon][listitem], stData[stid][stAmmo][listitem]);

				SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s has taken a \"%s\" from their weapon storage.", ReturnName(playerid), ReturnWeaponName(stData[stid][stWeapon][listitem]));

				stData[stid][stWeapon][listitem] = 0;
				stData[stid][stAmmo][listitem] = 0;

				Storage_Save(stid);
				Storage_Weapon(playerid, stid);
				stData[stid][stCountWeap] -= 1;
			}
			else
			{
				new
				...
				stData[stid][stWeapon][listitem] = weaponid;
				stData[stid][stAmmo][listitem] = ammo;
	...
	if(dialogid == HOUSE_CLOTHES)
	{
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					ShowPlayerDialog(playerid, HOUSE_CLOTHES1, DIALOG_STYLE_LIST, "House Clothes", "Simpan baju\nAmbil baju", "Select", "Back");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, HOUSE_CLOTHES2, DIALOG_STYLE_LIST, "House Clothes", "Simpan baju\nAmbil baju", "Select", "Back");
				}
				case 2:
				{
					ShowPlayerDialog(playerid, HOUSE_CLOTHES3, DIALOG_STYLE_LIST, "House Clothes", "Simpan baju\nAmbil baju", "Select", "Back");
				}
			}
		}
		else House_OpenStorage(playerid, houseid);
	...
	if(dialogid == HOUSE_CLOTHES1)
	{
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					if(hData[houseid][hSkin1] > 0)
						return Error(playerid, "Kamu tidak bisa menyimpan baju Di slots ini");

					if(pData[playerid][pSkin] == 154 || pData[playerid][pSkin] == 140)
						return Error(playerid, "Kamu tidak bisa menyimpan Baju ini");

					hData[houseid][hSkin1] = pData[playerid][pSkin];
					if(pData[playerid][pGender] == 1)
					{
					...
					Info(playerid, "Anda berhasil menyimpan Baju");
				}
				case 1: 
				{
					if(hData[houseid][hSkin1] == 0)
						return Error(playerid, "Tidak ditemukan baju di Slots ini");

					if(pData[playerid][pGender] == 1)
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin1]);
					}
					else
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin1]);
					}
					hData[houseid][hSkin1] = 0;
					Info(playerid, "Anda berhasil mengambil Baju");
				}
			}
		}
		else House_OpenStorage(playerid, houseid);
	...
	if(dialogid == HOUSE_CLOTHES2)
	{
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					if(hData[houseid][hSkin2] > 0)
						return Error(playerid, "Kamu tidak bisa menyimpan baju Di slots ini");

					if(pData[playerid][pSkin] == 154 || pData[playerid][pSkin] == 140)
						return Error(playerid, "Kamu tidak bisa menyimpan Baju ini");

					hData[houseid][hSkin2] = pData[playerid][pSkin];
					if(pData[playerid][pGender] == 1)
					{
						SetPlayerSkin(playerid, 154);
					}
				...
				case 1: 
				{
					...
					if(pData[playerid][pGender] == 1)
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin2]);
					}
					else
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin2]);
					}
					hData[houseid][hSkin2] = 0;
					Info(playerid, "Anda berhasil mengambil Baju");
				}
			}
		}
	...
	if(dialogid == HOUSE_CLOTHES3)
	{
		...
		if(response)
		{
			switch (listitem)
			{
				case 0: 
				{
					if(hData[houseid][hSkin3] > 0)
						return Error(playerid, "Kamu tidak bisa menyimpan baju Di slots ini");

					if(pData[playerid][pSkin] == 154 || pData[playerid][pSkin] == 140)
						return Error(playerid, "Kamu tidak bisa menyimpan Baju ini");

					hData[houseid][hSkin3] = pData[playerid][pSkin];
					if(pData[playerid][pGender] == 1)
					{
						SetPlayerSkin(playerid, 154);
					}
				...
				case 1: 
				{
					...
					if(pData[playerid][pGender] == 1)
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin3]);
					}
					else
					{
						SetPlayerSkin(playerid, hData[houseid][hSkin3]);
					}
					hData[houseid][hSkin3] = 0;
					Info(playerid, "Anda berhasil mengambil Baju");
				}
			}
		}
	...
	if(dialogid == HOUSE_WITHDRAW_REALMONEY)
	{
		...
		if(response)
		{
			new amount =  floatval(strval(inputtext));
	...
	if(dialogid == DIALOG_FIND_DEALER)
	{
		if(response)
		{
			new bid = ReturnDealershipID((listitem + 1));

			pData[playerid][pGpsActive] = 1;
			SetPlayerRaceCheckpoint(playerid,1, DealerData[bid][dealerPosX], DealerData[bid][dealerPosY], DealerData[bid][dealerPosZ], 0.0, 0.0, 0.0, 3.5);
			Gps(playerid, "Dealer Checkpoint targeted! (%s)", GetLocation(wsData[bid][wX], wsData[bid][wY], wsData[bid][wZ]));
		}
	}
		...
		if(response) 
		{
			...
			foreach(new i : PVehicles)
			{
				if(carid == pvData[i][cVeh])
				{
					...
					format(pvData[i][cPlate], 32, "Levana Pride-%d", rand);
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 3:
				{
	...
	//=== Check Starterpack
	if (dialogid == DIALOG_CHECKSTARTERPACK)
    {
        if (!response) return 1;  // Jika pemain menekan "Cancel", keluar dari handler dialog.

        new targetid = strval(inputtext);  // Mengonversi input ke player ID.

        // Cek apakah player ID valid.
        if (targetid < 0 || targetid >= MAX_PLAYERS)
        {
            SendClientMessage(playerid, -1, "{BABABA}INFO: {FFFFFF}Invalid player ID.");
            return 1;
        }

        // Cek apakah target pemain terhubung.
        if (!IsPlayerConnected(targetid))
        {
            SendClientMessage(playerid, -1, "{BABABA}INFO: {FFFFFF}Player is not connected.");
            return 1;
        }

        // Cek status starter pack dari player target.
        if (pData[targetid][pStarterpack] == 0)
        {
            SendClientMessage(playerid, -1, "{BABABA}INFO: {FFFFFF}Player belum menggambil starterpack");
        }
        else
        {
            SendClientMessage(playerid, -1, "{BABABA}INFO: {FFFFFF}Player sudah menggambil starterpack");
            pData[targetid][pStarterpack] = 0;  // Mengubah data starter pack menjadi 0 setelah diambil
        }
        return 1;
    }
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			...
			{
				new str[3500];
				strcat(str, "{BABABA}PLAYER: {ffffff}/help /afk /drag /undrag /pay /stats /items /frisk /use /give /idcard /drivelic /togphone /reqloc\n");
				strcat(str, "{BABABA}PLAYER: {ffffff}/weapon /settings /ask /answer /mask /helmet /death /accept /deny /revive /buy /health /destroycp /phone\n");
				strcat(str, "{BABABA}TWITTER: {ffffff}/togtw /tw /race /rpschalleng /rps /rpsaccept\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Player Commands", str, "Close", "");
			}
			case 1:
			{
				new str[3500];
				strcat(str, ""GREY_E"CHAT: {ffffff}/b /l /t /s /pm /togpm /w /o /me /ame /do /ado /try /ab\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Chat Commands", str, "Close", "");
			}
			case 2:
			{
				new str[3500];
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/engine - Toggle Engine || /light - Toggle lights\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/hood - Toggle Hood || /trunk - Toggle Trunk\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/lock - Toggle Lock || /myinsu - Check Insu\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/tow - Tow Vehicle || /untow - Untow Vehicle\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/mypv - Check Vehicles || /claimpv - Claim Insurance\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/buyplate - Buy Plate || /buyinsu - Buy Insurance\n");
				strcat(str, ""GREY_E"VEHICLE:  {ffffff}/eject\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Vehicle Commands", str, "Close", "");
			}
			case 3:
			{
				new line3[500];
				strcat(line3, "{15D4ED}JOB: {ffffff}Taxi\n{15D4ED}JOB: {ffffff}Mechanic\n{15D4ED}JOB: {ffffff}Lumber Jack\n{15D4ED}JOB: {ffffff}Trucker\n{15D4ED}JOB: {ffffff}Miner\n{15D4ED}JOB: {ffffff}Production\n{15D4ED}JOB: {ffffff}Farmer\n{15D4ED}JOB: {ffffff}Courier\n{15D4ED}JOB: {ffffff}Baggage Airport\n{FF0000}ILEGGALJOB: {ffffff}Smuggler\n{D2D2AB}ACTIVITY: {ffffff}Berry");
			...
			{
				new str[3500];
				strcat(str, ""GREY_E"BUSINESS: {ffffff}/buy /bm /lockbisnis /unlockbisnis /mybisnis\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Business Commands", str, "Close", "");
			}
			case 6:
			{
				new str[3500];
				strcat(str, ""GREY_E"HOUSE: {ffffff}/buy /storage /lockhouse /unlockhouse /myhouse\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "House Commands", str, "Close", "");
			}
			case 7:
			{
				new str[3500];
				strcat(str, ""GREY_E"WORKSHOP: {ffffff}/buy /wsmenu /myworkshop /service\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Workshop Commands", str, "Close", "");
			}
			case 8:
			{
				new str[3500];
				strcat(str, ""GREY_E"WORKSHOP: {ffffff}/buydealer /dealermanage /givedealer\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Dealership Commands", str, "Close", "");
			}
			case 9:
			{
				new str[3500];
				strcat(str, ""GREY_E"VENDING:{ffffff} /vendingmanage /buy\n");
				strcat(str, ""GREY_E"VENDING: klik '"RED_E"ENTER{ffffff}' atau '"RED_E"F{ffffff}' untuk membeli.\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Vending Commands", str, "Close", "");
			}
			case 10:
			{
				new str[3500];
				strcat(str, ""GREY_E"PRIVATE STORAGE: {ffffff}/buystorage /storagemenu\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Private Storage Commands", str, "Close", "");
			}
			case 11:
			{
				new str [3500];
				strcat(str, ""GREY_E"PRIVATE GARAGE: {ffffff}/mygarage /givegarage\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Private Garage Commands", str, "Close", "");
	...
	if(dialogid == DIALOG_JOB)
    {
		...
		switch(listitem)
		{
			...
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Unity Station\n\n{BABABA}CMDS: {ffffff}//taxiduty /fare\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Taxi Job", str, "Close", "");
			}
			case 1:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Idlewood\n\n{BABABA}CMDS: {ffffff}//mechduty /service\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Mechanic Job", str, "Close", "");
			}
			case 2:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini khusus untuk Lumber Profesional\n\n{BABABA}CMDS: {ffffff}//(lum)ber\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Lumber Job", str, "Close", "");
			}
			case 3:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Flint Country\n\n{BABABA}CMDS: {ffffff}/mission /storeproduct /storegas /storestock /creat /gps\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Trucker Job", str, "Close", "");
			}
			case 4:
			{
				new str[3500];
			...
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Flint Country arah Angel Pine\n\n{BABABA}CMDS: {ffffff}//createproduct /sellproduct\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Production Job", str, "Close", "");
			}
			case 6:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Flint Country\n\n{BABABA}CMDS: {ffffff}//plant /price /offer\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Farmer Job", str, "Close", "");
			}
			case 7:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini bisa anda cek Lewat GPS\n\n{BABABA}CMDS: {ffffff}/taruhbox\nSaat mulai bekerja kurir anda akan diperlihatkan Guide\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Courier Job", str, "Close", "");
			}
			case 8:
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Bandara Los Santos\n\n{BABABA}CMDS: {ffffff}//startbg\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Baggage Job", str, "Close", "");
			}
			case 9:
			{
				new str[3500];
				strcat(str, "Pekerjaan ini ileggal lokasi dan tempat cari sendiri sampai ketemu\n\n{BABABA}CMDS: {ffffff}//trackpacket /atrackpacket /pickuppacket /respawnpacket \n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Smuggler Job", str, "Close", "");
			}
			case 10:
			{
				new str[3500];
				strcat(str, "Pekerjaan ini dapat di lakukan saat kamu bergabung menjadi farmer\n\n{BABABA}CMDS: {ffffff}/berry\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Berry Activity", str, "Close", "");
			}
		}
	}			
	if(dialogid == DIALOG_GPS)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 5:
				{
					ShowPlayerDialog(playerid, DIALOG_GPS_MISSION, DIALOG_STYLE_LIST, "GPS My Mission", "My Mission (Bisnis)\nMy Hauling (Gas Station)\nMy Hauling (Dealership)", "Select", "Back");
				}
			}
		}
	}
	if(dialogid == DIALOG_GPS_JOB)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					SetPlayerRaceCheckpoint(playerid,1,  1685.35, -1464.57, 13.9719, 0.0,0.0,0.0, 3.5); //Taxi
					Gps(playerid, "Active!");
				}
				case 1:
				{
					SetPlayerRaceCheckpoint(playerid,1, 2281.08, -2365.03, 13.8469, 0.0, 0.0, 0.0, 3.5); //Mechanic City
					Gps(playerid, "Active!");
				}
				case 2:
				{
				...
				case 12:
				{
					SetPlayerRaceCheckpoint(playerid,1, 1789.1580,-1704.6975,13.5279, 0.0, 0.0, 0.0, 3.5); //kurir
					Gps(playerid, "Active!");
				}
				case 13:
				{
					SetPlayerRaceCheckpoint(playerid,1, -578.5306,-1507.9203,9.7769, 0.0, 0.0, 0.0, 3.5); //pemeras susus sapi
					Gps(playerid, "Active!");
				}
			}
		}
	...
	if(dialogid == DIALOG_GPS_MISSION)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 2:
				{
					if(pData[playerid][pDealerMission] == -1) return Error(playerid, "You dont have hauling.");
					new dealerid = pData[playerid][pDealerMission];
					Gps(playerid, "Follow the hauling checkpoint to find your dealership location.");
					SetPlayerRaceCheckpoint(playerid,1, DealerData[dealerid][dealerPointX], DealerData[dealerid][dealerPointY], DealerData[dealerid][dealerPointZ], 0.0, 0.0, 0.0, 3.5);
				}
			}
		}
		else 
	...
	if(dialogid == DIALOG_GPS_GENERAL)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1123.8778,-2036.9189,69.8858, 0.0, 0.0, 0.0, 3);//city hall
					Gps(playerid, "Active!");
				}
				case 2:
				{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 2028.0742,-1402.8625,17.2367, 0.0, 0.0, 0.0, 3);//asgh
					Gps(playerid, "Active!");
				}
				case 4:
				{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 333.9579,-1826.9429,4.0067, 0.0, 0.0, 0.0, 3);//pedagang
					Gps(playerid, "Active!");
				}
			}
		}
	...
	if(dialogid == DIALOG_GPS_PUBLIC)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 2792.800293, -1087.574097, 30.718750, 0.0, 0.0, 0.0, 3);//Insurance
					Gps(playerid, "Active!");
				}
				case 7:
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 2196.7, -2221.55, 13.8547, 0.0, 0.0, 0.0, 3);//Mechanic
					Gps(playerid, "Active!");
				}
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1339.68, 324.57, 20.27, 0.0, 0.0, 0.0, 3);//Component Shop
					Gps(playerid, "Active!");
				}
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1219.27, -1811.7, 16.8937, 0.0, 0.0, 0.0, 3);//Auctioned
					Gps(playerid, "Active");
				}
	...
	if(dialogid == DIALOG_LOCKERSAPD)
	{
		if(response)
		{
			switch (listitem) 
			{
				...
				case 6:
				{
					if(pData[playerid][pDeviceType] == DEVICE_MOBILE)
							return Error(playerid, "Fitur ini tidak tersedia untuk perangkat mobile!");
					...
					switch (pData[playerid][pGender])
					{
						case 1: ShowPlayerSelectionMenu(playerid, SAPD_SKIN_03DL, "Choose Your Skin", SAPDSkin03DL, sizeof(SAPDSkin03DL));
					}
				}
			}
		}
	...
	if(dialogid == DIALOG_LOCKERSAMD)
	{
		if(response)
		{
			switch (listitem) 
			{
				...
				case 6:
				{
					ShowPlayerDialog(playerid, DIALOG_INPUTOBATSAMD, DIALOG_STYLE_INPUT, "Obat Myricous SAMD", "Masukan jumlah yang ingin di ambil:", "Input", "Batal");
				}
			}
		}
		return 1;
	}
	if(dialogid == DIALOG_INPUTOBATSAMD)
	{
		if(response)
		{
			new amount = strval(inputtext);

			if(amount <= 0 || amount > 5) return Error(playerid, "Invalid Amount! Max 5!");
			if(pData[playerid][pMedicine] >= 5) return Error(playerid, "Telah Memenuhi Batas pengambilan Medicine, Max 5!");
			
			pData[playerid][pMedicine] += amount;
			Info(playerid, "Berhasil Mengambil Medicine sebanyak %d dari locker SAMD", amount);
		}
	}
...
//					else return Error(playerid, "You must in Mechanic Center Area!");
	...
	if(dialogid == ST_WITHDRAWMONEY)
	{
		if(response)
		{
			new amount = floatval(strval(inputtext)), stid = pData[playerid][pInSt];
		...
		if(response)
		{
			switch(listitem)
			{
				...
				{  
					...
					if(pData[playerid][pMaterial] < 320) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}320{ffffff}).");
					if(pData[playerid][pMoney] < 2500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$2.500{ffffff}).");
					
					pData[playerid][pMaterial] -= 320;
					pData[playerid][pMoney] -= 2500;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}320 Material {ffffff}dan membayar uang senilai {7fff00}$2,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_SILENCED, 250);
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pMaterial] < 250) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}250{ffffff}).");
					if(pData[playerid][pMoney] < 3500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$8.000,0{ffffff}).");
					
					pData[playerid][pMaterial] -= 250;
					pData[playerid][pMoney] -= 3500;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}320 {ffffff}dan membayar uang senilai {7fff00}$3,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_COLT45, 300);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pMaterial] < 300) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}300{ffffff}).");
					if(pData[playerid][pMoney] < 4500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: $4.500{ffffff}).");
					
					pData[playerid][pMaterial] -= 300;
					pData[playerid][pMoney] -= 4500;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}320 {ffffff}dan membayar uang senilai {7fff00}$4,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_SHOTGUN, 200);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
//					if(pData[playerid][pFamily]) return Error(playerid, "Kamu harus meiliki FNG untuk membuat senjata ini");
					if(pData[playerid][pMaterial] < 350) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}350{ffffff}).");
					if(pData[playerid][pMoney] < 5000) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$5.000{ffffff}).");
					
					pData[playerid][pMaterial] -= 350;
					pData[playerid][pMoney] -= 5000;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}320 Material {ffffff}dan membayar uang senilai {7fff00}$5,000");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_DEAGLE, 150);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
				case 4: //ak-47
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
//					if(pData[playerid][pFamily]) return Error(playerid, "Kamu harus meiliki FNG untuk membuat senjata ini");
					if(pData[playerid][pMaterial] < 500) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}500{ffffff}).");
					if(pData[playerid][pMoney] < 6500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$6.500{ffffff}).");
					
					pData[playerid][pMaterial] -= 500;
					pData[playerid][pMoney] -= 6500;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}320 Material {ffffff}dan membayar uang senilai {7fff00}$6,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_AK47, 500);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
...
//					if(pData[playerid][pFamily]) return Error(playerid, "Kamu harus meiliki FNG untuk membuat senjata ini");
					if(pData[playerid][pMaterial] < 500) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}500{ffffff}).");
					if(pData[playerid][pMoney] < 7500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$7.500{ffffff}).");
					
					pData[playerid][pMaterial] -= 500;
					pData[playerid][pMoney] -= 7500;
					
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}500 Material {ffffff}dan membayar uang senilai {7fff00}$7,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_SAWEDOFF, 200);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
				case 6: //UZI
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
//					if(pData[playerid][pFamily]) return Error(playerid, "Kamu harus meiliki FNG untuk membuat senjata ini");
					if(pData[playerid][pMaterial] < 400) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}400{ffffff}).");
					if(pData[playerid][pMoney] < 5000) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$5.000{ffffff}).");
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}400 Material {ffffff}dan membayar uang senilai {7fff00}$5,000");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_UZI, 500);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
...
//					if(pData[playerid][pFamily]) return Error(playerid, "Kamu harus meiliki FNG untuk membuat senjata ini");
					if(pData[playerid][pMaterial] < 500) return Error(playerid, "Material tidak cukup!(Butuh: {FF0000}500{ffffff}).");
					if(pData[playerid][pMoney] < 5500) return Error(playerid, "Uang kamu tidak cukup!(Butuh: {7fff00}$5.500{ffffff}).");

					pData[playerid][pMaterial] -= 500;
					pData[playerid][pMoney] -= 5500;

					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan {FF0000}500 Material {ffffff}dan membayar uang senilai {7fff00}$5,500");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_MP5, 500);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
	...
	if(dialogid == DIALOG_HAULING)
	{
		if(response)
		{
			...
			pData[playerid][pTrailer] = AddStaticVehicleEx(584, 326.57, 857.31, 20.40, 290.67, -1, -1, -1, 0);
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2:
				{
					...
					Server_AddMoney(200);
					Info(playerid, "Anda membeli bandage seharga "RED_E"$200");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_BANK)
	{
		...
		switch(listitem)
		{
			...
			{
				new mstr[512];
				format(mstr, sizeof(mstr), "{F6F6F6}You have {7fff00}%s {F6F6F6}in bank account.\n\nType in the amount you want to deposit below:", FormatMoney(pData[playerid][pBankMoney]));
				ShowPlayerDialog(playerid, DIALOG_BANKDEPOSIT, DIALOG_STYLE_INPUT, ""GREY3_E"Bank", mstr, "Deposit", "Cancel");
			}
			case 1: // Withdraw
			{
				new mstr[512];
				format(mstr, sizeof(mstr), "{F6F6F6}You have {7fff00}%s {F6F6F6}in your bank account.\n\nType in the amount you want to withdraw below:", FormatMoney(pData[playerid][pBankMoney]));
				ShowPlayerDialog(playerid, DIALOG_BANKWITHDRAW, DIALOG_STYLE_INPUT, ""GREY3_E"Bank", mstr, "Withdraw", "Cancel");
			}
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			mysql_tquery(g_SQL, query);
			format(lstr, sizeof(lstr), "{F6F6F6}You have successfully deposited {7fff00}%s {F6F6F6}into your bank account.\n"LB_E"Current Balance: {F6F6F6}%s", FormatMoney(amount), FormatMoney(pData[playerid][pBankMoney]));
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""GREY3_E"Bank", lstr, "Close", "");
		}
	}
	if(dialogid == DIALOG_BANKWITHDRAW)
	{
		...
		else
		{
			...
			mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET bmoney=%d,money=%d WHERE reg_id=%d", pData[playerid][pBankMoney], pData[playerid][pMoney], pData[playerid][pID]);
			mysql_tquery(g_SQL, query);
			format(lstr, sizeof(lstr), "{F6F6F6}You have successfully withdrawed {7fff00}%s {F6F6F6}from your bank account.\n"LB_E"Current Balance: {F6F6F6}%s", FormatMoney(amount), FormatMoney(pData[playerid][pBankMoney]));
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""GREY3_E"Bank", lstr, "Close", "");
		}
	}
	if(dialogid == DIALOG_BANKREKENING)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_BANKTRANSFER, DIALOG_STYLE_INPUT, ""WHITE_E"Bank", "Masukan nomor rekening target:", "Transfer", "Cancel");
		}
	}
	...
	if(dialogid == DIALOG_BANKCONFIRM)
	{
		if(response)
		{
			...
			ShowPlayerDialog(playerid, DIALOG_BANKSUKSES, DIALOG_STYLE_MSGBOX, ""WHITE_E"Transfer Sukses", mstr, "Sukses", "");
			SendStaffMessage(COLOR_RED, "[TRANSFER] "WHITE_E"%s transfer to %s ammount "GREEN_E"%s", pData[playerid][pName], pData[playerid][pTransferName], FormatMoney(pData[playerid][pTransfer]));
		}
	}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_BUYPVCP_BIKES, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Motorcycle", str, "Buy", "Close");
				}
				case 1:
				{
					//Cars
					...
					ShowPlayerDialog(playerid, DIALOG_BUYPVCP_CARS, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Ordinary Vehicle", str, "Buy", "Close");
				}
				case 2:
				{
					//Unique Cars
					...
					ShowPlayerDialog(playerid, DIALOG_BUYPVCP_UCARS, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Vehicle Unique", str, "Buy", "Close");
				}
				case 3:
				{
					//Job Cars
					...
					ShowPlayerDialog(playerid, DIALOG_BUYPVCP_JOBCARS, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Vehicle Job", str, "Buy", "Close");
				}
				case 4:
				{
					// VIP Cars
					new str[1024];
					format(str, sizeof(str), "Vehicle\tPrice\n"WHITE_E"%s\t\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s\t"YELLOW_E"%d Coin\n%s", 
					...
					ShowPlayerDialog(playerid, DIALOG_BUYPVCP_VIPCARS, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}VIP Vehicle", str, "Buy", "Close");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_SWEEPER)
	{
		...
		if(response)
		{
			ShowPlayerDialog(playerid, DIALOG_RUTE_SWEEPER, DIALOG_STYLE_LIST, "Pilih Rute Sweeper", "Route A {7fff00}$300\n{ffffff}Route B {7fff00}$350", "Pilih", "Batal");
		}
		else
		{
			RemovePlayerFromVehicle(playerid);
	...
	if(dialogid == DIALOG_BUS)
	{
		...
		if(response)
		{
			ShowPlayerDialog(playerid, DIALOG_RUTE_BUS, DIALOG_STYLE_LIST, "Pilih Rute Bus", "Route A {7fff00}$650\n{ffffff}Route B {7fff00}$750", "Pilih", "Batal");
		}
		else
		{
			RemovePlayerFromVehicle(playerid);
			...
			switch(listitem)
			{
				case 0://Rute 1
				{
				    if(DialogBaggage[0] == false) // Kalau False atau tidak dipilih
				    {
						...
						pData[playerid][pTrailerBaggage] = AddStaticVehicleEx(606, 2137.2085, -2380.0925, 13.2078, 180.7874, 1, 1, -1);
				...
				case 1://Rute 2
				{
				    if(DialogBaggage[1] == false) // Kalau False atau tidak dipilih
				    {
						...
						pData[playerid][pTrailerBaggage] = AddStaticVehicleEx(607, 2009.4430, -2273.0322, 13.2024, 91.8682, 1, 1, -1);
				...
				case 2://Rute 3
				{
				    if(DialogBaggage[2] == false) // Kalau False atau tidak dipilih
				    {
						...
						pData[playerid][pTrailerBaggage] = AddStaticVehicleEx(607, 1897.6689, -2225.1143, 13.2150, 180.8993, 1, 1, -1);
	...
	if(dialogid == WS_WITHDRAWMONEY)
	{
		if(response)
		{
			new amount =  floatval(strval(inputtext)), id = pData[playerid][pInWs];
	...
	if(dialogid == DIALOG_VENDING_PRICESET)
	{
		...
		if(PlayerOwnVending(playerid, vid))
		{
			if(response)
			{
				...
				{
					new str[128];
					format(str,sizeof(str), "Please enter the new product price for %s ($1,0 to $50):", item);
					ShowPlayerDialog(playerid, DIALOG_VENDING_PRICESET, DIALOG_STYLE_INPUT, "Vending: Set Price", str, "Modify", "Back");
					return 1;
				}
	...
	if(dialogid == DIALOG_VERIFYCODE)
	{
		if(response)
		{
			...
			if(strval(inputtext) == pData[playerid][pVerifyCode])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Levana Pride", lstring, "Register", "Quit");	
				return 1;
			}
	...
	if(dialogid == DIALOG_PHONE_TEKS)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					return callcmd::gpsteks(playerid);
				}
				case 1:
				{
					return callcmd::callteks(playerid);
				}
				case 2:
				{
					return callcmd::smsteks(playerid);
				}
				case 3:
				{
					return callcmd::contactteks(playerid);
				}
				case 4:
				{
					new tstr[800];
		...
		switch(listitem)
		{
			case 0:
			{
				...
				else
				{
					...
					format(line9, sizeof(line9), "{ffffff}[{7348EB}INFO VEHICLE{ffffff}]:\nVehicle UID: {ffff00}%d\n{ffffff}Model: {ffff00}%s\n{ffffff}Plate: {ffff00}%s{ffffff}\n\n{ffffff}[{7348EB}DATA VEHICLE{ffffff}]:\nInsurance: {ffff00}%d{ffffff}\nPark ID: {ffff00}%d{ffffff}",
					pvData[vid][cID], GetVehicleModelName(pvData[vid][cModel]), pvData[vid][cPlate], pvData[vid][cInsu], pvData[vid][cPark]);
	...
	if(dialogid == DIALOG_SERVERMONEY_STORAGE)
	{
		...
		else 
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Levana Pride Money", lstr, "Manage", "Close");
		}
	}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
	                ...
	                vehicleid = CreateVehicle(596, 1602.9182,-1644.2645,13.3311,2.6847,0,1, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 1:
				{
	                ...
	                vehicleid = CreateVehicle(407,1602.9182,-1644.2645,13.3311,2.6847,0,1, 1800);
                    PolisiVeh{vehicleid} = true;
				...
				case 2:
				{
	                ...
	                vehicleid = CreateVehicle(544,1602.9182,-1644.2645,13.3311,2.6847,0,1, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 3:
				{
	                ...
	                vehicleid = CreateVehicle(599,1602.9182,-1644.2645,13.3311,2.6847,0,1, 1800);
                    PolisiVeh{vehicleid} = true;
				...
				case 4:
				{
	                ...
	                vehicleid = CreateVehicle(601,1602.9182,-1644.2645,13.3311,2.6847,1,1, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 5:
				{
	                ...
	                vehicleid = CreateVehicle(528,1602.9182,-1644.2645,13.3311,2.6847,0,0, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 6:
				{
	                ...
	                vehicleid = CreateVehicle(560,1602.9182,-1644.2645,13.3311,2.6847,0,0, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 7:
				{
	                ...
	                vehicleid = CreateVehicle(523,1602.9182,-1644.2645,13.3311,2.6847,0,0, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 8:
				{
	                ...
	                vehicleid = CreateVehicle(525,1602.9182,-1644.2645,13.3311,2.6847,18,20, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 9:
				{
	                ...
	                vehicleid = CreateVehicle(497,1574.8530,-1690.9780,23.2872,86.5594,0,1, 1800);
					PolisiVeh{vehicleid} = true;
				...
				case 10:
				{
	                ...
	                vehicleid = CreateVehicle(541,1602.9182,-1644.2645,13.3311,2.6847,0,1, 1800);
					PolisiVeh{vehicleid} = true;
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
	                ...
	                vehicleid = CreateVehicle(407, 2033.2816,-1428.9435,17.0132,180.1070, -1, 3, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 1:
				{
	                ...
	                vehicleid = CreateVehicle(563, 2032.8159,-1380.3557,48.3359,90.4886, -1, 3, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 2:
				{
	                ...
	                vehicleid = CreateVehicle(544, 2033.2816,-1428.9435,17.0132,180.1070, -1, 3, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 3:
				{
	                ...
	                vehicleid = CreateVehicle(416, 2033.2816,-1428.9435,17.0132,180.1070, 1, 3, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 4:
				{
	                ...
	                vehicleid = CreateVehicle(586, 2033.2816,-1428.9435,17.0132,180.1070, 0, 1, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 5:
				{
	                ...
	                vehicleid = CreateVehicle(426, 2033.2816,-1428.9435,17.0132,180.1070, 0, 1, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 6:
				{
	                ...
	                vehicleid = CreateVehicle(442, 2033.2816,-1428.9435,17.0132,180.1070, 0, 1, 1800);
					SamdVeh{vehicleid} = true;
				...
				case 7:
				{
	                ...
	                vehicleid = CreateVehicle(487, 2032.8159,-1380.3557,48.3359,90.4886, -1, 3, 1800);
					SamdVeh{vehicleid} = true;

                    PutPlayerInVehicle(playerid, vehicleid, vehicleid);
                    SetVehicleNumberPlate(vehicleid, "SAMD");
				}
			}
		}
	...
	if(dialogid == DIALOG_TRACK_PH)
	{
		if(response)
		{
			new playerToTrackID = strval(inputtext); // ID player untuk di-track

			// Pastikan ID player valid dan terhubung
			if(playerToTrackID < 0 || playerToTrackID >= MAX_PLAYERS || !IsPlayerConnected(playerToTrackID))
			{
				return Error(playerid, "Invalid player ID. Please enter a valid player ID.");
			}

			// Pengecekan apakah player memiliki handphone dan handphone aktif
			if(pData[playerid][pPhone] == 0)
			{
				return Error(playerid, "You do not have a phone. Please get a phone first!");
			}
			
			// Pengecekan status handphone player yang akan di-track
			if(pData[playerToTrackID][pPhoneStatus] == 0)
			{
				return Error(playerid, "The player's phone is currently turned off.");
			}

			// Melanjutkan dengan tracking
			Info(playerid, "Processing Track for phone number, Please Wait");
			SetTimerEx("trackph", random(10000) + 1, false, "iid", playerid, playerToTrackID);
			return 1;
		}
	}
	...
	if(dialogid == DIALOG_DISNAKER)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 0:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Pekerjaan 2");
							pData[playerid][pJob2] = 3; 
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Lumberjack");
				...
				case 1:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Penebang Kayu");
							pData[playerid][pJob2] = 1; 
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Taxi");
				...
				case 2:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Trucker");
							pData[playerid][pJob2] = 4;
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Trucker");
				...
				case 3:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Miner");
							pData[playerid][pJob2] = 5; 
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Miner");
				...
				case 4:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Product");
				...
				case 5:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Farmer");
							pData[playerid][pJob2] = 7; 
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Farmer");
				...
				case 6:
				{
					if (pData[playerid][pJob] > 0) {
						...
						if (pData[playerid][pJob2] > 0) {
							...
							Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Baggage");
							pData[playerid][pJob2] = 10; 
						}
					} else {
						Info(playerid, "Anda Memilih Pekerjaan Sebagai ~p~Baggage");
				...
				case 7:
				{
				    if(pData[playerid][pJob] == 0) return Error(playerid, "Anda seorang pengangguran.");
				    if(pData[playerid][pJob] == 2)
					{
					    //mekanik--;
					    pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~mekanik");
					...
					else if(pData[playerid][pJob] == 10)
					{
						...
						Info(playerid, "Anda resign menjadi ~p~Baggage");
					}
					else if(pData[playerid][pJob] == 3)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~lumberjack");
					}
					else if(pData[playerid][pJob] == 5)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~Miner.");
					...
					else if(pData[playerid][pJob] == 1)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~Taxi");
					}
					else if(pData[playerid][pJob] == 4)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~Trucker");
					}
					else if(pData[playerid][pJob] == 6)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~Product");
					}
					else if(pData[playerid][pJob] == 7)
					{
				    	...
				    	pData[playerid][pJob] = 0;
						pData[playerid][pJob2] = 0;
						Info(playerid, "Anda resign menjadi ~p~Farmer");
					}
				}
			}
		}
	...
	if(dialogid == DIALOG_LISTVEHGO)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialog(playerid, DIALOG_MOTORCYCLEGO, DIALOG_STYLE_LIST, "Motorcycle List", "PCJ-600\nSanchez\nBF-400", "Spawn", "Close");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, DIALOG_CARGO, DIALOG_STYLE_LIST, "Car List", "Premier\nPrevion\nMoonbeam\nBravura\nMr. Whoopee", "Spawn", "Close");
				}
			}
		}
	}
	if(dialogid == DIALOG_CARGO)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 0:
				{
					new color1 = RandomEx(1, 10);
					new color2 = RandomEx(1, 10);
					new Float: x,
		    			Float: y,
						Float: z,
						Float: a;
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(426, x, y, z, a, color1, color2, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 1:
				{
					new color1 = RandomEx(1, 10);
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(436, x, y, z, a, color1, color2, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 2:
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(418, x, y, z, a, color1, color2, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 3:
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(401, x, y, z, a, color1, color2, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 4:
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(423, x, y, z, a, color1, color2, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
			}
		}
	}
	if(dialogid == DIALOG_MOTORCYCLEGO)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 0: 
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(461, x, y, z, a, 16, 16, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 1: 
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(468, x, y, z, a, 16, 16, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
				case 2:
				{
					...
					GetPlayerPos(playerid, x, y, z);
	    			GetPlayerFacingAngle(playerid, a);
					pData[playerid][pSpawnGo] = AddStaticVehicleEx(581, x, y, z, a, 16, 16, VEHICLE_RESPAWN, 1);
					PutPlayerInVehicle(playerid, pData[playerid][pSpawnGo], 0);
				}
			}
		}
