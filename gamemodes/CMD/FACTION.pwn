...
CMD:divorced(playerid, params[])
{
	...
	if(sscanf(params, "u", giveplayerid)) return SendClientMessage(playerid, COLOR_WHITE, "/cerai [playerid]");
...
CMD:marry(playerid, params[])
{
	...
	{
		SendClientMessageEx(playerid, COLOR_GREY, "The Marriage & Reception costs $1,000!");
		return 1;
	}
	if(pData[playerid][pMarried] > 0)
	{
	...
	if(sscanf(params, "u", giveplayerid)) return SendClientMessage(playerid, COLOR_WHITE, "/marry [playerid]");
...
CMD:factionhelp(playerid)
{
	...
	{
		new str[3500];
		strcat(str, ""WHITE_E"SAPD: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank\n");
		strcat(str, ""WHITE_E"SAPD: /sapdonline /(un)cuff /tazer /detain /arrest /release /flare /destroyflare /checkveh /takedl\n");
		strcat(str, ""WHITE_E"SAPD: /takemarijuana /spike /destroyspike /destroyallspike /getloc /rb\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""BLUE_E"SAPD", str, "Close", "");
	}
	else if(pData[playerid][pFaction] == 2)
	{
		new str[3500];
		strcat(str, ""WHITE_E"SAGS: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank\n");
		strcat(str, ""WHITE_E"SAGS: /sagsonline /(un)cuff /checkcitymoney\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E2"SAGS", str, "Close", "");
	}
	else if(pData[playerid][pFaction] == 3)
	{
		new str[3500];
		strcat(str, ""WHITE_E"SAMD: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank\n");
		strcat(str, ""WHITE_E"SAMD: /samdonline /loadinjured /dropinjured /ems /findems /healbone /rescue /salve /mix /treatment\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK_E"SAMD", str, "Close", "");
	}
	...
	else if(pData[playerid][pFaction] == 5)
	{
		...
		strcat(str, ""WHITE_E"Pedagang/Go Car: /menumasak /gofare /od /d /m \n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
	...
	else if(pData[playerid][pFamily] != -1)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""DARK_E"Family", str, "Close", "");
	}
	else
	{
...
CMD:gov(playerid, params[])
{
	...
	else if(pData[playerid][pFaction] == 5)
	{
		...
		SendClientMessageToAll(COLOR_WHITE, "|___________ Government News Announcement ___________|");
		SendClientMessageToAll(COLOR_YELLOW, lstr);
	}
	return 1;
}
...
CMD:release(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0,  1546.63, 1350.38, 1510.91))
...
CMD:givelic(playerid, params[])
{
	...
	new String[10000], giveplayerid, jenis;
	if(sscanf(params, "us[455]", giveplayerid, jenis))
		//return Usage(playerid, "/givelic [playerid] [1 = weapon, 2 = truck, 3 = cgc]");
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "KEGUNAAN: /givelic [playerid] [jenis]");
		SendClientMessageEx(playerid, COLOR_GRAD2, "Jenis: 1 = Weapon, 2 = Trucker, 3 = CGC");
		return 1;
	}

	if(giveplayerid == INVALID_PLAYER_ID || !NearPlayer(playerid, giveplayerid, 4.0))
	...
	switch(jenis)
		{
			case 1:
			{
				if(pData[giveplayerid][pWeaponLic] == 1)
				{
				...
				SendClientMessageEx(giveplayerid, COLOR_ARWIN, "NOTE: "WHITE_E"Your {00FFFF}Weapon license "WHITE_E"is active for "YELLOW_E"2 weeks");   
				pData[giveplayerid][pWeaponLic] = 1;
				pData[giveplayerid][pWeaponLicTime] = gettime() + (14 * 86400);
				return 1;
			}
		    ...
		    case 2:
			{
				...
				SendClientMessageEx(giveplayerid, COLOR_WHITE, String);
				SendClientMessageEx(giveplayerid, COLOR_ARWIN, "NOTE: "WHITE_E"Your {00FFFF}Trucking license "WHITE_E"is active for "YELLOW_E"2 weeks");   
				pData[giveplayerid][pTruckerLic] = 1;
				pData[giveplayerid][pTruckerLicTime] = gettime() + (14 * 86400);
				return 1;
			}
		}
	return 1;
...
CMD:dropinjured(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 5.0, 2042.3931,-1416.2369,17.1641))
    {
...
//     if(IsPlayerInRangeOfPoint(playerid, 2.0, 1832.69, -1075.17, 41.65))
//     {
...
public DutyHour(playerid)
{
	...
	if(pData[playerid][pDutyHour] == 3600)
	{
		if(pData[playerid][pFaction] == 1)
		{
			AddPlayerSalary(playerid, "Duty(SAPD)", 500);
			pData[playerid][pDutyHour] = 0;
			Servers(playerid, "Anda telah Duty selama 1 Jam dan Anda mendapatkan Pending Salary anda");
		}
		else if(pData[playerid][pFaction] == 2)
		{
			AddPlayerSalary(playerid, "Duty(SAGS)", 500);
			pData[playerid][pDutyHour] = 0;
			Servers(playerid, "Anda telah Duty selama 1 Jam dan Anda mendapatkan Pending Salary anda");
		}
		else if(pData[playerid][pFaction] == 3)
		{
			AddPlayerSalary(playerid, "Duty(SAMD)", 500);
			pData[playerid][pDutyHour] = 0;
			Servers(playerid, "Anda telah Duty selama 1 Jam dan Anda mendapatkan Pending Salary anda");
		}
		else if(pData[playerid][pFaction] == 4)
		{
			AddPlayerSalary(playerid, "Duty(SANEWS)", 500);
			pData[playerid][pDutyHour] = 0;
			Servers(playerid, "Anda telah Duty selama 1 Jam dan Anda mendapatkan Pending Salary anda");
		}
	}
...
CMD:sapdvehicle(playerid) //sapd vehicle spawn
{
    ...
    if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1602.9182,-1644.2645,13.3311)) return Error(playerid, "Anda harus berada di garasi sapd!");
	ShowPlayerDialog(playerid, DIALOG_PVSAPD, DIALOG_STYLE_LIST, "SAPD VEHICLE", "Police LS\nFire truck\nFire Truck\nPolice Ranger\nS.W.A.T\nFbi Truck\nSultan\nHPV1000\nTow Truck\nPolice Maveric\nBullet", "Ambil", "Cancel");
    return 1;
}

CMD:sapdparking(playerid, params[])
{
	if(pData[playerid][pFaction] != 1)
	   return Error(playerid, "You dont have Faction Sapd!");
    if(!IsPlayerInRangeOfPoint(playerid, 3.0,  1602.9182,-1644.2645,13.3311)) return Error(playerid, "Anda harus berada di garasi sapd!");
...
CMD:samdvehicle(playerid, params[]) //sapd vehicle destroy
{
	if(pData[playerid][pFaction] != 3)
	   return Error(playerid, "You dont have Faction Samd!");
    if(!IsPlayerInRangeOfPoint(playerid, 3.0, 2033.2816,-1428.9435,17.0132)) return Error(playerid, "Anda harus berada di garasi samd!");
...
CMD:samdparking(playerid, params[])
{
	if(pData[playerid][pFaction] != 3)
	   return Error(playerid, "You dont have Faction Sapd!");
    if(!IsPlayerInRangeOfPoint(playerid, 3.0,  2014.4462,-1412.3612,16.9922)) return Error(playerid, "Anda harus berada di garasi sapd!");
	if(pData[playerid][pInjured] >= 1) return Error(playerid, "Kamu tidak bisa melakukan ini!");
	if(!IsPlayerInAnyVehicle(playerid)) return Error(playerid, "You must be in Vehicle");
    new vehicleid = GetPlayerVehicleID(playerid);
...
CMD:facdespawnpolisi(playerid, params[]){
	if(pData[playerid][pAdmin] < 2) return PermissionError(playerid);

	new maxVehicles = GetVehiclePoolSize(); // Mendapatkan jumlah maksimum kendaraan yang didukung
    for (new i = 1; i <= maxVehicles; i++) {
		
		if(AdaOrangDiKendaraan(PolisiVeh{i})) continue;

		DestroyVehicle(i);
		PolisiVeh{i} = false;
		Info(playerid, "Mobil Polisi yang tidak di gunakan telah di destroy!");
	}
	return 1;
}

CMD:facdespawnsamd(playerid, params[]){
	if(pData[playerid][pAdmin] < 2) return PermissionError(playerid);

	new maxVehicles = GetVehiclePoolSize(); // Mendapatkan jumlah maksimum kendaraan yang didukung
    for (new i = 1; i <= maxVehicles; i++) {

		if(AdaOrangDiKendaraan(SamdVeh{i})) continue;
