...
ptask PlayerDelay[1000](playerid)
{
	...
	if(pData[playerid][pVip] > 0)
	{
	    ...
	    //GYM Membership
	if(pData[playerid][pGymVip] > 0)
    {
       if(pData[playerid][pGymTime] != 0 && pData[playerid][pGymTime] <= gettime())
       {
           Info(playerid, "GYM Membership anda telah berakhir");
           pData[playerid][pGymVip] = 0;
           pData[playerid][pGymTime] = 0;
       }
    }
	...
	{
		pData[playerid][pExitJob]--;
	}
	if(pData[playerid][pTagsTime] > 0)
	{
		pData[playerid][pTagsTime]--;
	}
