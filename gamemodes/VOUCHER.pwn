...
CMD:createredeemcode(playerid, params[])
{
	if(pData[playerid][pAdmin] < 5)
		return PermissionError(playerid);
	
	new voucid = Iter_Free(Vouchers), query[128];
	if(voucid == -1) return Error(playerid, "You cant create more redeem code!");
	new code, vip, viptime, gold, money;
	if(sscanf(params, "ddddd", code, vip, viptime, money, gold)) return Usage(playerid, "/createredeemcode [CODE(73821)] [VIP] [VIP-TIME(Days)] [MONEY] [GOLD]");
	...
	foreach(new vo : Vouchers)
	{
		...
		{
			return Error(playerid, "Redeem code already registered! try another code!");
		}
	}
	...
	mysql_tquery(g_SQL, query, "OnRedeemcodeCreated", "i", voucid);
	
	Servers(playerid, "Redeem code created id: %d | code: %d | vip: %d | vip-time: %d | money: %s | gold: %d | admin: %s", voucid, VoucData[voucid][voucCode], VoucData[voucid][voucVIP], VoucData[voucid][voucVIPTime], FormatMoney(VoucData[voucid][voucMoney]), VoucData[voucid][voucGold], pData[playerid][pAdminname]);
	return 1;
}
...
CMD:redeemcode(playerid, params[])
{
	new code;
	if(sscanf(params, "d", code)) return Usage(playerid, "/redeemcode [CODE]");
		...
		if(VoucData[vo][voucCode] == code)
		{
			if(VoucData[vo][voucClaim] == 0)
			{
				if(VoucData[vo][voucVIP] == 0)
				{
					...
					Info(playerid, "Redeem code claimed. gold: %d | claimby: %s.", VoucData[vo][voucGold], pData[playerid][pName]);
				}
				else
				{
					...
					Info(playerid, "Redeem code claimed. VIP: %d | VIP TIME: %d days | money: %s | gold: %d | claimby: %s.", VoucData[vo][voucVIP], dayz, FormatMoney(VoucData[vo][voucMoney]), VoucData[vo][voucGold], pData[playerid][pName]);
				}
			}
			else return Error(playerid, "Redeem code has been expired!");
		}
	}
	return 1;
}
