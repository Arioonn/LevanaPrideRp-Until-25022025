...
House_Save(houseid)
{
	...
	format(cQuery, sizeof(cQuery), "%s, houseint='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', intposx='%f', intposy='%f', intposz='%f', intposa='%f', visit='%d', redmoney='%d', snack='%d', sprunk='%d', medicine='%d', medkit='%d', bandage='%d', seed='%d', material='%d', marijuana='%d', component='%d',  houseskin1='%d',  houseskin2='%d',  houseskin3='%d' WHERE ID='%d'",
		...
		hData[houseid][hSkin1],
		hData[houseid][hSkin2],
		hData[houseid][hSkin3],
        houseid
    );
...
House_Type(houseid)
{
	if(hData[houseid][hType] == 1)
	{
	    switch(random(3))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = 243.718;
				hData[houseid][hIntposY] = 304.936;
				hData[houseid][hIntposZ] = 999.448;
				hData[houseid][hIntposA] = 89.0975;
				hData[houseid][hInt] = 1;
			}
			case 1:
			{
	...
	if(hData[houseid][hType] == 2)
	{
	    switch(random(5))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = 2807.63;
				hData[houseid][hIntposY] = -1174.76;
				hData[houseid][hIntposZ] = 1025.87;
				hData[houseid][hIntposA] = 179.284;
	...
	if(hData[houseid][hType] == 3)
	{
	    switch(random(2))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = 2324.41;
				hData[houseid][hIntposY] = -1149.55;
				hData[houseid][hIntposZ] = 1051.01;
				hData[houseid][hIntposA] = 183.02;
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_int(i, "houseskin1", hData[hid][hSkin1]);
			cache_get_value_name_int(i, "houseskin2", hData[hid][hSkin2]);
			cache_get_value_name_int(i, "houseskin3", hData[hid][hSkin3]);
