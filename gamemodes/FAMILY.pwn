...
Family_Refresh(id)
{
	if(id != -1)
	{
		...
		format(mstr,sizeof(mstr),"\n{BABABA}%s\n{BABABA}Press {FF0000}ENTER {FFFFFF}to enter", fData[id][fName]);
		fData[id][fPickext] = CreateDynamicPickup(1254, 23, fData[id][fExtposX], fData[id][fExtposY], fData[id][fExtposZ], 0, 0, -1, 50);
        ...
        {
            format(mstr,sizeof(mstr),"\n{BABABA}%s\n{FFFFFF}Press {FF0000}ENTER {FFFFFF}to exit", fData[id][fName]);
            ...
            fData[id][fPickint] = CreateDynamicPickup(1254, 23, fData[id][fIntposX], fData[id][fIntposY], fData[id][fIntposZ], id, fData[id][fInt], -1, 50);
        }
		if(fData[id][fSafeposX] != 0.0 && fData[id][fSafeposY] != 0.0 && fData[id][fSafeposZ] != 0.0)
		{
			format(tstr, sizeof(tstr), "\n{BABABA}Family Storage", id);
            ...
            fData[id][fPicksafe] = CreateDynamicPickup(19832, 23, fData[id][fSafeposX], fData[id][fSafeposY], fData[id][fSafeposZ], id, fData[id][fInt], -1, 50);
		}
	}
}
...
CMD:flist(playerid, params[])
{
	...
	// }	
	new string[256];
	format(string, sizeof(string), "Nama Family (ID)\tFamily Leader\n");
	for(new i = 0; i < MAX_FAMILY; i ++)
	{
		format(string, sizeof(string), "%s %s(%d)\t%s", string, fData[i][fName], i, fData[i][fLeader]);
	}
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "Family List", "Close", "");
	return 1;
}
