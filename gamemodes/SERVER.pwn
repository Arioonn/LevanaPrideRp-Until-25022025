...
CreateServerPoint()
{
	...
	PedagangPickup = CreateDynamicPickup(1239, 23, 348.76, -1842.20, 4.01, -1, -1, -1, 5.0);
	format(strings, sizeof(strings), "[Information]\n"WHITE_E"Pedagang Stock: "LG_E"%d", Pedagang);
	PedagangText = CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 348.76, -1842.20, 4.01, 5.0); // pedagang

	CompPickup = CreateDynamicPickup(1239, 23, 1339.68, 324.57, 20.27, -1, -1, -1, 50.0);
	...
	CompText = CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 1339.68, 324.57, 20.279, 5.0); // comp
...
{
    return floatval((float(price) / 10000) * 8500);
}
...
Server_Save()
{
	...
	CreateServerPoint();
    format(str, sizeof(str), "UPDATE server SET servermoney='%d', material='%d', materialprice='%d', lumberprice='%d', component='%d', componentprice='%d', metalprice='%d', gasoil='%d', gasoilprice='%d', coalprice='%d', product='%d', productprice='%d', medicineprice='%d', medkitprice='%d', food='%d', foodprice='%d', seedprice='%d', potatoprice='%d', wheatprice='%d', orangeprice='%d', fishprice='%d', gstationprice='%d', obatmyr='%d', obatprice='%d', pedagang='%d' WHERE id=0",
	...
	Food, FoodPrice, SeedPrice, PotatoPrice, WheatPrice, OrangePrice, FishPrice, GStationPrice, ObatMyr, ObatPrice, Pedagang);
